...
class Person2 extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('person_model2','person2');
	...
	public function index()
	{
		...
		$this->load->view('person_view2');
	}

	public function ajax_list()
	{
		$list = $this->person2->get_datatables();
		$data = array();
		$no = $_POST['start'];
		...
		$output = array(
						"draw" => $_POST['draw'],
						"recordsTotal" => $this->person2->count_all(),
						"recordsFiltered" => $this->person2->count_filtered(),
						"data" => $data,
				);
	...
	{
		$data = $this->person2->get_by_id($id);
	...
	public function ajax_update()
	{
		...
		$this->person2->update(array('id' => $this->input->post('id')), $data);
		echo json_encode(array("status" => TRUE));
	}

	public function ajax_delete($id)
	{
		$this->person2->delete_by_id($id);
		echo json_encode(array("status" => TRUE));
	}
